...
<head>
    ...
    <title>Laporan Saldo Piutang</title>
    <style>
...
<body>
    <div class="container">
        ...
        <div class="report-title text-center">LAPORAN SALDO PIUTANG</div>
        ...
        <table>
            <thead>
                <tr>
                    ...
                    <th style="width: 15%;" class="text-right">Saldo Piutang</th>
                </tr>
            </thead>
            <tbody>
                @php($no = 1)
                ...
                @foreach($customers as $customer)
                    @if($customer->balance > 0)
                        @php($totalPiutang += $customer->balance)
                        <tr>
                            <td>{{ $no++ }}</td>
                            <td><strong>{{ $customer->name }}</strong></td>
                            <td>{{ $customer->phone ?? '-' }}</td>
                            <td>{{ $customer->address ?? '-' }}</td>
                            <td class="text-right">Rp {{ number_format($customer->balance, 0, ',', '.') }}</td>
                        </tr>
                    @endif
                @endforeach
                @if($no === 1)
                    <tr>
                        <td colspan="5" class="text-center" style="padding: 20px; color: #666;">Tidak ada saldo piutang</td>
                    </tr>
                @endif
            </tbody>
        </table>
