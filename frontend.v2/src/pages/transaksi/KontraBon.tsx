...
import { useDownloadKontraBonPdf, useTransactions } from '@/hooks/api/useTransactions';
...
const KontraBon = () => {
  ...
  const { user } = useAuth();
  const downloadKontraBonPdf = useDownloadKontraBonPdf();
  ...
  const buildFilename = useCallback((customerName: string) => {
    ...
    return `kontra-bon-${slug || 'customer'}-${date}.pdf`;
  }, []);

  const handleDownloadCustomer = useCallback(async (customerId: string, group: { name: string; items: Transaction[] }) => {
    if (customerId === 'unknown') {
      toast({ title: 'Customer tidak valid untuk export PDF', variant: 'destructive' });
      return;
    }

    try {
      await downloadKontraBonPdf.mutateAsync({
        customerId,
        transactionIds: group.items.map(item => item.id),
        filename: buildFilename(group.name),
      });
  ...
  const handleDownloadAll = useCallback(async () => {
    if (printableGroups.length === 0) {
      toast({ title: 'Tidak ada data valid untuk diunduh', variant: 'destructive' });
      return;
    }

    setIsDownloading(true);
    try {
      for (const [customerId, group] of printableGroups) {
        await downloadKontraBonPdf.mutateAsync({
          customerId,
          transactionIds: group.items.map(item => item.id),
          filename: buildFilename(group.name),
      ...
      const skippedCount = filteredGrouped.length - printableGroups.length;
      if (skippedCount > 0) {
        toast({
          title: 'Selesai dengan catatan',
          description: `${printableGroups.length} PDF diunduh, ${skippedCount} grup dilewati karena customer tidak valid.`,
        });
      } else {
  ...
  }, [buildFilename, downloadKontraBonPdf, filteredGrouped.length, printableGroups, toast]);
           ...
           <div className="flex gap-2">
               {canPrint('transactions.kontra_bon') && (
                  <>
                   ...
                   <Button variant="outline" className="h-8 text-xs gap-1.5" onClick={handleDownloadAll} disabled={isDownloading || downloadKontraBonPdf.isPending}>
                     <Printer className="h-3.5 w-3.5" />{isDownloading ? 'Mengunduh...' : 'Cetak Semua'}
                   </Button>
                 </>
               )}
            </div>
                      ...
                      {canPrint('transactions.kontra_bon') && (
                        <div className="flex justify-end mt-3">
                          <Button
                            ...
                            onClick={() => handleDownloadCustomer(cid, group)}
                            disabled={cid === 'unknown' || isDownloading || downloadKontraBonPdf.isPending}
                          >
