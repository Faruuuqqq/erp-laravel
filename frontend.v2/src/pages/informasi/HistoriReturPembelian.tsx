...
import { Eye, EyeOff, FileDown, ChevronLeft, ChevronRight, RotateCcw, Printer, Loader2 } from 'lucide-react';
...
const HistoriReturPembelian = () => {
  ...
  const handleDownloadInvoice = useCallback(async (t: Transaction) => {
    ...
    try {
      await downloadPdfMutation.mutateAsync({
        transactionId: t.id,
        filename: `${t.invoiceNumber ?? 'retur'}.pdf`,
        documentType: 'invoice',
      });
      toast({ title: 'Berhasil', description: `Dokumen ${t.invoiceNumber} diunduh` });
    } catch {
  ...
  const handleExportXlsx = useCallback(async () => {
    ...
    try {
      const headers = ['No. Retur', 'Tanggal', 'Supplier', 'Nilai Retur', 'Alasan'];
      ...
      await exportTransactionsToXlsx({
        filename: getFilenameWithDate('histori-retur-pembelian'),
        headers,
        data,
        currencyColumns: [3],
        rightAlignColumns: [3],
             ...
             <Table>
               ...
               <TableBody>
                 {isLoading ? (
                   ...
                   <TableRow><TableCell colSpan={7} className="text-center py-10 text-muted-foreground">Tidak ada data retur pembelian</TableCell></TableRow>
                 ) : filtered.map(t => (
              ...
              <div className="flex justify-end gap-2">
                ...
                >
                  {downloadingId === selectedTrx?.id
                    ...
                    : <><Printer className="mr-1.5 h-3.5 w-3.5" />Cetak Dokumen</>}
                </Button>
